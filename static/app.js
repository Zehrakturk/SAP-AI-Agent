...
const App = (() => {
  ...
  const ROUTES = [
    ...
    { id: 'documents',    label: 'Belgeler',        icon: documentsIcon(),    page: () => DocumentsPage    },
    { id: 'metrics',      label: 'Metrikler',       icon: metricsIcon(),      page: () => MetricsPage, admin: true },
  ...
  const PAGE_META = {
    ...
    documents   : { title: 'Belgeler',           crumb: 'PDF Bilgi Tabanı' },
    metrics     : { title: 'Metrikler',          crumb: 'Semantic Layer' },
      ...
      <aside class="sidebar">
        ...
        <div class="sidebar-user">
          ...
          <div class="user-info">
            ...
            <div class="user-role">${user.role}${user.company && user.company !== 'ALL' ? ' · ' + user.company : ' · Tüm Firmalar'}</div>
          </div>
        </div>
        <button onclick="App.logout()" title="Çıkış yap"
          style="display:flex;align-items:center;justify-content:center;gap:8px;
                 width:calc(100% - 24px);box-sizing:border-box;margin:4px 12px 14px;
                 padding:10px;background:rgba(239,68,68,0.08);
                 border:1px solid rgba(239,68,68,0.25);border-radius:8px;color:#dc2626;
                 font-size:13px;font-weight:600;font-family:inherit;cursor:pointer;transition:all .18s"
          onmouseover="this.style.background='#dc2626';this.style.color='#fff';this.style.borderColor='#dc2626'"
          onmouseout="this.style.background='rgba(239,68,68,0.08)';this.style.color='#dc2626';this.style.borderColor='rgba(239,68,68,0.25)'">
  ...
  function renderNav() {
    return ROUTES.filter(r => !r.admin || _isAdmin()).map(r => `
  ...
  function logout() {
    ...
    if (!confirm(`Portaldan çıkış yapılsın mı?${who}`)) return;
    Auth.logout(() => {
      // Arka plan zamanlayıcılarını durdur
      if (_notifTimer) { clearInterval(_notifTimer); _notifTimer = null; }
      user = null;
      // Uygulama kabuğunu temizle → login ekranına dön (tam sayfa yenilemeden)
      document.body.innerHTML = '';
      Auth.showLogin((u) => {
        user = u;
        renderShell();
  ...
  function documentsIcon() { return `<svg width="18" height="18" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>`; }
  function metricsIcon() { return `<svg width="18" height="18" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><line x1="4" y1="20" x2="4" y2="10"/><line x1="10" y1="20" x2="10" y2="4"/><line x1="16" y1="20" x2="16" y2="14"/><line x1="22" y1="20" x2="2" y2="20"/></svg>`; }
})();
