...
**Sürüm:** 1.0 · **Tarih:** 2026-06 · **Durum:** Aktif geliştirme (Sprint 1–3 + Human-in-the-loop tamamlandı)
...
2. Soru OpenAI ile embed edilir; **Qdrant**'ta en alakalı entegrasyon şema(lar)ı bulunur (RAG).
3. **Intent parser** soruyu yapılandırır (tarih aralığı, metrik, gruplama, filtre).
4. **GPT-4o** şema + niyet bağlamıyla güvenli bir T-SQL SELECT üretir; `_is_safe_sql` denetler.
5. Sorgu **MSSQL**'de çalışır. Sonuç varsa GPT analiz metni + KPI + Chart.js grafiği üretir;
   **Gemini** arka planda görseli/raporu zenginleştirir.
6. Sonuç yoksa **Human-in-the-loop** devreye girer: kullanıcıya "Bu veri yok, entegrasyon
   çalışsın mı?" sorulur → Evet → admin onayı → onaylanınca SAP'tan çekilir → sorgu otomatik
   yeniden çalışır → cevap teslim edilir.
7. Gece arka planda **proaktif içgörüler** üretilir; kullanıcı ertesi sabah ana sayfada
...
| Rol | Örnek | Yetkiler |
|-----|-------|----------|
| **ADMIN** | `admin` | Sorgu + entegrasyon yönetimi + veri çekme onayı (approve/reject) + tüm ayarlar |
| **VIEWER** | `p.frank` | Sorgu sorar, rapor üretir, içgörü görür; onay veremez, entegrasyon yönetemez |
...
## 4. Kapsam
**Dahil (tamamlanan):**
- Doğal dil → T-SQL sorgu (RAG destekli)
- Çok-turlu sohbet (son 3 mesaj bağlamı, "aynısını Mart için" çözümü)
- Proaktif içgörü motoru (5 detector + hipotez üretimi)
...
- Gerçek JWT/SSO kimlik doğrulama, satır-seviyesi RBAC
...
**Veri katmanı:** MSSQL (`ZehraTestDB`, ODBC Driver 18, pyodbc) + Qdrant vektör DB (koleksiyon `SAP-AI`).
**AI servisleri:**
- OpenAI **GPT-4o** — NL→SQL üretimi + analiz/yorum
- OpenAI **text-embedding-3-small** — RAG embedding (1536 boyut, cosine)
- Google **Gemini 2.0 Flash** — rapor & görsel zenginleştirme
...
- **Repository:** insights, ingestion (approval/job/audit)

**Katmanlar:** API (Flask blueprints) → Services (`query_engine`, `fetchers`, `insights`,
`ingestion`, `report_renderer`, `email_sender`, `gemini_enhancer`) → Repositories → MSSQL/Qdrant.

```
Kullanıcı → Flask API → query_engine ──RAG──> Qdrant (şema arama)
                              │
                              ├─> MSSQL (T-SQL çalıştır)
                              ├─> GPT-4o (SQL + analiz)
                              ├─> Gemini (zenginleştirme)
                              └─> ingestion (eksik veri → onay → SAP fetch)
...
### 6.1 Kimlik (auth)
Demo login: kullanıcı/şifre doğrulanır → `demo-token-{id}-{role}` döner. Rol her istekte
token'dan okunur. (Üretim için gerçek JWT planlı — bkz. §11.)

### 6.2 Sorgu Motoru (query_engine)
RAG (soru embed → Qdrant'ta şema ara) → intent parse (tarih/metrik/grup) → T-SQL üret →
güvenlik denetimi (`_is_safe_sql`, yalnız SELECT/WITH) → MSSQL'de çalıştır → GPT analiz +
Chart.js paketi (KPI, highlights, primary/secondary grafik). Çok-turlu: son 3 mesaj bağlama
...
sonuç otomatik kullanıcıya teslim. Tüm geçişler append-only `audit_log`'da. Dedup, retry
(exponential backoff), atomik job claim, stale-lock kurtarma.
...
│   │   ├── query.py             # /query/ask — sohbet sorgusu girişi
│   │   ├── integrations.py      # entegrasyon CRUD + şablonlar + indeksleme
...
│   │   ├── query_engine.py      # RAG + intent + SQL üretim + analiz (çekirdek)
│   │   ├── qdrant_indexer.py    # şema embedding → Qdrant
│   │   ├── gemini_enhancer.py   # Gemini çağrıları
...
│   ├── repositories/
│   │   └── integration_repository.py   # integrations → IntegrationConfig DTO
│   └── models/store.py          # demo USERS, SETTINGS, AI_LOGS (in-memory)
├── static/                      # Vanilla JS frontend (sayfa başına modül)
│   ├── chats.js insights.js integrations.js reports.js dashboard.js
│   └── auth.js settings.js users.js logs.js charts.js tables.js api_client.js
...
| Tablo | Amaç |
|-------|------|
| `shipments` | SAP sevkiyat verisi (TKNUM, ERDAT, MUSTERI_ADI, CITY1, LFIMG, TDURUM …) |
| `integrations` | Entegrasyon tanımları (tip, url, auth, kimlik) |
...
| `chat_sessions` | Sohbet oturumları (+ `tags`, `pinned`) |
| `chat_messages` | Sohbet mesajları (rol, içerik, data_json) |
...
## 8. API Referansı (prefix `/api/v1`, ~75 route)
...
**approvals (HITL):** `POST /approvals/request`, `GET /approvals/`, `GET /approvals/<id>`,
`POST /approvals/<id>/approve`, `POST /approvals/<id>/reject`,
`GET /approvals/status/<external_id>`, `GET /approvals/jobs/<id>`, `POST /approvals/run-worker`
...
| `GEMINI_API_KEY` | `AIza...` | Gemini 2.0 Flash zenginleştirme |
...
`SQL_MODEL` (vars. `gpt-4o`), `ANALYSIS_MODEL` (vars. `gpt-4o`). Embedding sabit: `text-embedding-3-small` (1536 boyut).
...
| Kullanıcı | Şifre | Rol | id | Departman |
|-----------|-------|-----|----|-----------|
| `admin` | `admin123` | ADMIN | 1 | IT |
| `p.frank` | `view123` | VIEWER | 8 | Management |

- **Token formatı:** `demo-token-{id}-{role}` → admin için `demo-token-1-ADMIN`.
...
- HITL'de çözülen sonuç şu an `chat_messages`'a kalıcı yazılmıyor (sayfa yenilenince
  onay kartı kalır, cevap kaybolur) — v2 adayı.
- Kimlik/RBAC demo seviyesinde (bkz. §11).

---

## 15. Yol Haritası (Sprint 4+)
- **Semantic Layer** (metrik sözlüğü — SQL doğruluğunu önemli ölçüde artırır)
- Otomatik follow-up soru önerileri
